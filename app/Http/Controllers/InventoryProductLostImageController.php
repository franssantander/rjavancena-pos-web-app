...
class InventoryProductLostImageController extends Controller
{
    protected $helper, $fillable_attr_product_lost_image;

    public function __construct(Helper $helper, InventoryProductLostImageModel $fillable_attr_product_lost_image)
    {
        $this->helper = $helper;
    ...
    {
        $crud_settings = $this->fillable_attr_product_lost_image->getApiCrudSettings();
        $relative_settings = $this->fillable_attr_product_lost_image->getApiRelativeSettings();
        ...
        // $inventory_product_losts = InventoryProductLostImageModel::orderBy('created_at', 'desc')->get();

        // Define the number of items per page
        $per_page = 10; // Set how many notifications you want per page

        // Fetch paginated inventory items for the current page
        $inventory_product_lost_images = InventoryProductLostImageModel::orderBy('created_at', 'desc') // Ensure results are ordered for consistent pagination
        ...
        foreach ($inventory_product_lost_images as $inventory_product_lost_image) {
            ...
            foreach ($this->fillable_attr_product_lost_image->getFillableAttributes() as $getFillableAttribute) {
                // fields to encrypt
                if (in_array($getFillableAttribute, $this->fillable_attr_product_lost_image->arrToConvertIdsToEncrypted())) {
                    $arr_container_datas[$getFillableAttribute] = Crypt::encrypt($inventory_product_lost_image->$getFillableAttribute);
                }
                // fields to convert date and time
                else if (in_array($getFillableAttribute, $this->fillable_attr_product_lost_image->arrToConvertToReadableDateTime())) {
                    $arr_container_datas[$getFillableAttribute] = $this->helper->convertReadableTimeDate($inventory_product_lost_image->$getFillableAttribute);
                }
                // just declare
                else {
                    $arr_container_datas[$getFillableAttribute] = $inventory_product_lost_image->$getFillableAttribute;
                }
            }
            ...
            foreach ($arr_container_datas['actions'] as &$action) {
                ...
                if ($action['button_name'] == 'Edit') {
                    ...
                    foreach ($this->fillable_attr_product_lost_image->arrFieldsUpdate() as $key => $arrDetails) {
                ...
                $action['inventory_product_lost_image_id'] = $arr_container_datas['inventory_product_lost_image_id'] ?? null;
            }

            // ***************************** //

            // Unset the fields not to use
            foreach ($this->fillable_attr_product_lost_image->arrFieldsToUnsetTable() as $arrFieldsToUnsetTable) {
        ...
        $response = [
            ...
            'columns' => $this->helper->transformColumnName($this->fillable_attr_product_lost_image->arrFieldsColumnHeaderTable()),
            ...
            'pagination' => [
                'count' => $inventory_product_lost_images->count(),
                'has_page' => $inventory_product_lost_images->hasPages(),
                'has_more_pages' => $inventory_product_lost_images->hasMorePages(),
                'current_page' => $inventory_product_lost_images->currentPage(),
                'last_page' => $inventory_product_lost_images->lastPage(),
                'per_page' => $inventory_product_lost_images->perPage(),
                'next_page_url' => $inventory_product_lost_images->nextPageUrl(),
                'previous_page_url' => $inventory_product_lost_images->previousPageUrl(),
            ],
        ];
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $inventory_product_lost_id = InventoryProductLostModel::where('inventory_product_lost_id', $decrypted_inventory_product_lost_id)->first();
            if (!$inventory_product_lost_id) {
                DB::rollBack();
                return response()->json(['message' => 'Inventory product lost ID not found.'], Response::HTTP_NOT_FOUND);
            }
            ...
            if ($request->hasFile('image') && $request->file('image')->isValid()) {
                ...
                $file_name = $this->helper->handleUploadFile(
                    [
                        'custom_folder' => 'inventory-product-lost-image',
            ...
            $result_to_create = $this->helper->arrStoreMultipleData(
                $this->fillable_attr_product_lost_image->arrToStores(),
                $request->all(),
                $file_name,
                $this->fillable_attr_product_lost_image->arrPayloadIdsToDecrypt(),
                [],
                []
            );


            $created = InventoryProductLostImageModel::create($result_to_create);
            if (!$created) {
                DB::rollBack();
                return response()->json(['message' => 'Failed to store product lost'], Response::HTTP_INTERNAL_SERVER_ERROR);
            }

            $update_unique_id = $this->helper->updateUniqueId($created, $this->fillable_attr_product_lost_image->idToUpdate(), Str::uuid());
            if ($update_unique_id) {
                DB::rollBack();
            ...
            $log_result = $this->helper->log(
                ...
                [
                    ...
                    'is_history' => 0,
                    'user_action' => 'STORE_INVENTORY_PRODUCT_LOST_IMAGE',
                ]
            );
        ...
        return response()->json(['message' => 'Product lost images stored successfully'], Response::HTTP_OK);
    }
    ...
    public function update(Request $request)
    {
        ...
        try {
            // Decrypted id
            $decrypted_inventory_product_lost_image_id = Crypt::decrypt($request->input('inventory_product_lost_image_id'));
            $decrypted_inventory_product_lost_id = Crypt::decrypt($request->input('inventory_product_lost_id'));

            // Check if voucher record exists
            $inventory_product_lost_image = InventoryProductLostImageModel::where('inventory_product_lost_image_id', $decrypted_inventory_product_lost_image_id)
                ->where('inventory_product_lost_id', $decrypted_inventory_product_lost_id)
                ->first();
            if (!$inventory_product_lost_image) {
            ...
            $result_changes_item_for_logs = $this->helper->updateLogsOldNew(
                $inventory_product_lost_image,
                $this->fillable_attr_product_lost_image->arrToUpdates(),
                $request->all(),
                $file_name
            );
            $changes_for_logs[] = [
            ...
            $log_result = $this->helper->log(
                ...
                [
                    ...
                    'user_id' => $user->user_id,
                    'is_history' => 0,
                    'user_action' => 'UPDATE_INVENTORY_PRODUCT_LOST_IMAGE',
                ]
            );
            if ($log_result->getStatusCode() !== Response::HTTP_OK) {
                DB::rollBack();
    ...
    public function destroy(Request $request)
    {
        ...
        try {
            ...
            foreach ($this->fillable_attr_product_lost_image->getFillableAttributes() as $getFillableAttributes) {
                $arr_log_details['fields'][$getFillableAttributes] = $inventory_product_lost_image->$getFillableAttributes;
            }

            // Delete the user
            if (!$inventory_product_lost_image->delete()) {
                DB::rollBack();
            ...
            $log_result = $this->helper->log(
                ...
                [
                    ...
                    'user_id' => $user->user_id,
                    'is_history' => 0,
                    'user_action' => 'DELETE_INVENTORY_PRODUCT_LOST_IMAGE',
                ]
            );
            if ($log_result->getStatusCode() !== Response::HTTP_OK) {
                DB::rollBack();
