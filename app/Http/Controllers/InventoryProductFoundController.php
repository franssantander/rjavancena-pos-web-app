...
class InventoryProductFoundController extends Controller
{
    protected $helper, $fillable_attr_product_found;

    public function __construct(Helper $helper, InventoryProductFoundModel $fillable_attr_product_found)
    {
        $this->helper = $helper;
    ...
    public function index(Request $request)
    {
        ...
        // $inventory_product_founds = InventoryProductFoundModel::orderBy('created_at', 'desc')->get();

        // Fetch paginated inventory items for the current page
        $inventory_product_founds = InventoryProductFoundModel::orderBy('created_at', 'desc') // Ensure results are ordered for consistent pagination
        ...
        foreach ($inventory_product_founds as $inventory_product_found) {
            ...
            foreach ($this->fillable_attr_product_found->getFillableAttributes() as $getFillableAttribute) {
                ...
                if (in_array($getFillableAttribute, $this->fillable_attr_product_found->arrToConvertIdsToEncrypted())) {
                    $arr_container_datas[$getFillableAttribute] = Crypt::encrypt($inventory_product_found->$getFillableAttribute);
                }
                // fields to convert date and time
                    ...
                    $arr_container_datas[$getFillableAttribute] = $this->helper->convertReadableTimeDate($inventory_product_found->$getFillableAttribute);
                }
                // just declare
                else {
                    $arr_container_datas[$getFillableAttribute] = $inventory_product_found->$getFillableAttribute;
                }
            }
            ...
            foreach ($arr_container_datas['actions'] as &$action) {
                ...
                if ($action['button_name'] == 'Edit') {
                    ...
                    foreach ($this->fillable_attr_product_found->arrFieldsUpdate() as $key => $arrDetails) {
        ...
        try {
            ...
            if ($request->hasFile('image') && $request->file('image')->isValid()) {
                $file_name = $this->helper->handleUploadFile(
                    [
                        'custom_folder' => 'inventory-product-found',
            ...
            $result_to_create = $this->helper->arrStoreMultipleData(
                $this->fillable_attr_product_found->arrToStores(),
                $result_merge_data,
                $this->fillable_attr_product_found->arrPayloadIdsToDecrypt(),
                [],
                [],
                [],
            );
            ...
            // Create 
            $created = InventoryProductFoundModel::create($result_to_create);
            if (!$created) {
            ...
            $update_unique_id = $this->helper->updateUniqueId(
                $created,
                $this->fillable_attr_product_found->idToUpdate(),
                Str::uuid()
            );
            ...
            $log_result = $this->helper->log(
                ...
                [
                    ...
                    'is_history' => 0,
                    'user_action' => 'STORE_INVENTORY_PRODUCT_FOUND',
                ],
                $created->toArray(),
                1,
                env("PATH_FILE_INVENTORY_PRODUCT_FOUND")
            );
            ...
            DB::commit();
            return response()->json(['message' => 'Product found stored successfully'], Response::HTTP_OK);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $inventory_product_found = InventoryProductFoundModel::where('inventory_product_found_id', $decrypted_inventory_product_found_id)
                ->where('inventory_product_id', $decrypted_inventory_product_id)
                ->first();
            if (!$inventory_product_found) {
            ...
            $inventory_product = $inventory_product->update([
                'stocks' => $inventory_product->stocks - $inventory_product_found->count,
            ]);
            //  ******************************* //
            ...
            $result_update_logs_old_new = $this->helper->updateLogsOldNew(
                $inventory_product_found, // the model to update
                $this->fillable_attr_product_found->arrToUpdates(), // fields to update
            ...
            $result_update_multi_data = $this->helper->arrUpdateMultipleData(
                ...
                $this->fillable_attr_product_found->arrToUpdates(),
                $result_merge_data,
                [],
                [],
                [],
            ...
            $log_result = $this->helper->log(
                ...
                [
                    ...
                    'user_id' => $user->user_id,
                    'is_history' => 0,
                    'user_action' => 'UPDATE_INVENTORY_PRODUCT_FOUND',
                ],
    ...
    public function destroy(Request $request)
    {
        ...
        try {
            ...
            $arr_log_details = $inventory_product_found->toArray();
            ...
            // Delete 
            if (!$inventory_product_found->delete()) {
                DB::rollBack();
                return response()->json(['message' => 'Failed to delete inventory product found'], Response::HTTP_UNPROCESSABLE_ENTITY);
            }
            ...
            $log_result = $this->helper->log(
                ...
                [
                    ...
                    'user_id' => $user->user_id,
                    'is_history' => 0,
                    'user_action' => 'DELETE_INVENTORY_PRODUCT_FOUND',
                ],
                $arr_log_details,
                3,
                env("PATH_FILE_INVENTORY_PRODUCT_RESTOCK")
